...
// extIx is the instruction implementation for Token-2022 extension builders.
type extIx struct {
	accounts []*solana.AccountMeta
	data     []byte
}

func (e *extIx) ProgramID() solana.PublicKey     { return ProgramID }
func (e *extIx) Accounts() []*solana.AccountMeta { return e.accounts }
func (e *extIx) Data() ([]byte, error)           { return e.data, nil }
...
func NewInitializeMintCloseAuthority(mint solana.PublicKey, closeAuth *solana.PublicKey) solana.Instruction {
	return &extIx{
		accounts: []*solana.AccountMeta{solana.NewAccountMeta(mint, false, true)},
		data:     encoding.New().U8(25).Raw(optNonZeroKey(closeAuth)).Bytes(),
	}
}
...
func NewInitializePermanentDelegate(mint, delegate solana.PublicKey) solana.Instruction {
	return &extIx{
		accounts: []*solana.AccountMeta{solana.NewAccountMeta(mint, false, true)},
		data:     encoding.New().U8(35).Raw(delegate[:]).Bytes(),
	}
}
...
func NewSetTransferFee(mint, authority solana.PublicKey, feeBasisPoints uint16, maximumFee uint64) solana.Instruction {
	return &extIx{
		...
		data: encoding.New().U8(26).U8(5).U16(feeBasisPoints).U64(maximumFee).Bytes(),
	}
}
...
func NewWithdrawWithheldTokensFromAccounts(mint, destination, withdrawAuthority solana.PublicKey, sources []solana.PublicKey) solana.Instruction {
	...
	return &extIx{
		accounts: accounts,
		data:     []byte{26, 3, byte(len(sources))},
	}
}
...
func NewHarvestWithheldTokensToMint(mint solana.PublicKey, sources []solana.PublicKey) solana.Instruction {
	...
	return &extIx{accounts: accounts, data: []byte{26, 4}}
}
...
func NewUpdateInterestBearingMintRate(mint, rateAuthority solana.PublicKey, rate int16) solana.Instruction {
	return &extIx{
		...
		data: encoding.New().U8(33).U8(1).I16(rate).Bytes(),
	}
}
